...
class TwoStreamPreprocessing(nn.Module):
    ...
    def compute_m_channel(
        ...
        Returns:
            m_channel: [B, 4, H, W], [ΔL, ΔR, ΔG, ΔB], range [-1, 1]
        """
        # Luminance change
        lum_prev = self.compute_luminance(image_previous)
        lum_curr = self.compute_luminance(image_current)
        delta_luminance = torch.clamp(lum_curr - lum_prev, -1, 1)

        # RGB channel changes
        delta_r = torch.clamp(
            image_current[:, 0:1] - image_previous[:, 0:1], -1, 1
        )
        delta_g = torch.clamp(
            image_current[:, 1:2] - image_previous[:, 1:2], -1, 1
        )
        delta_b = torch.clamp(
            image_current[:, 2:3] - image_previous[:, 2:3], -1, 1
        )

        return torch.cat([delta_luminance, delta_r, delta_g, delta_b], dim=1)
    ...
    def forward(
        ...
        Returns:
            m_channel: [B, 4, H, W] - temporal change
