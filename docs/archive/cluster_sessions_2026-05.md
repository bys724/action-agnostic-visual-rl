# 클러스터 세션 로그 — 2026-05 (archive)

> [docs/cluster_sessions.md](../cluster_sessions.md)에서 분리된 2026-05 entries. 누적 사용량은 main 문서 참조.
> 구 v15 CoRL 시점 기록 다수 (paper_experiments_plan §C1~C12 등은 삭제된 plan — git history 참조).
...
**fair pair 조건 (v15 본 학습과 동일)**: 50ep, EgoDex part1(⚠️ 2026-07-21 교정: 두 잡 로그 모두 `Splits: part1` 46,234 vids — 구 기재 part1-5는 오기), batch 32/GPU global 256, num_workers=8, λ_pred=λ_m_jepa=λ_compose=1.0 with warmup 10ep 0.01→1.0, composition_mode=linear_residual, EMA 0.999→0.9999, mask_p=0.75, mask_m_jepa=0.5, max_gap=30, sample_center=15. 단일 변인: `V11_ROUTING_MODE=v_from_m`.
