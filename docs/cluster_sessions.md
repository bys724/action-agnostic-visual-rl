...
| 36837641 | AIP_long 2×4 H100 | 2-00:00:00 | **🚀 4차: from-scratch 재학습** (07-15 사용자 결정) — 근거: 2·3차가 모두 1차 런의 ep5 ckpt 공유 = "3/3 재현"에 공유-초기조건 confound(1차 ep0-5 중 잠재 손상 가능성 미배제). env = 36829403 원 submit line 동일 + `CHECKPOINT_SUFFIX=fulldata_comp_mae_s_7ep_fs`(신규 dir → auto-resume 미발동, fresh init = 독립 초기조건; sampler seed 0 유지 = 데이터 순서 통제) + `SPIKE_GUARD_K=4.0`(보험, 가중치 보호용). 판별: 청정 완주 = 1차 런 고유 손상 쪽 + 그 자체가 §4-b 유효 측정 / 재발 = 시스템적 불안정 확정 → 서랍/스케줄변경. fresh start·`--spike-guard-k 4.0` 전달·31.5M 샘플 로드 확인 ✓ | 🔄 RUNNING (07-15 09:44~, 예상 ~16h ≈ 127 GPU·h) |
| 36837669 | core_s CPU | 00:10:00 | orchestrator 재장전 #3 (`afterok:36837641`, kill-on-invalid-dep) — **skip-게이트 신설**: 학습 로그 guard summary 합계 > 2,000(전체 step ~1%)이면 후속 제출 차단 (36835715 재발 방지, exit 0만으로 발동하던 구멍 봉합). 음성(sg 로그 49,093 skip → 차단)·양성(청정 로그 → 15잡) 테스트 PASS. `CKPT_BASE`·`TRAIN_LOG_GLOB` env로 `_fs` 런 지정 | PENDING (Dependency) |
