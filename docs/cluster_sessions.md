...
| 36833097 | AIP 2×4 H100 | 10:00:00 | **🚀 A안 수리 잡: ep5 재개 → ep6-7 재실행** (사용자 확정: seed 동일 = 재발 시 원인 진단). 시드 dir `20260714_ep5seed/latest.pt` = `checkpoint_epoch0005.pt` 복사(auto-resume glob 최신). env = 36829403 동일(EPOCHS=7). 재개 확인: `Resumed from epoch 5, LR 7.00e-05, starting from epoch 6` ✓. 예상 ~4.5h ≈ 36 GPU·h | 🔄 RUNNING (2026-07-14 14:2x, olaf-g[003-004]) |
| 36833098 | core_s CPU 1노드 | 00:10:00 | **orchestrator 재장전** (`--dependency=afterok:36833097`) — 수리 완료 시 청정 latest.pt(glob 최신)로 15잡 재제출 (EgoDex 3·OOD 8·SSv2 1-c 4) | 🔄 Dependency 대기 |
