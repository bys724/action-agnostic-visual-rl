...
- **🔴 Train↔inference 아키 parity — `strict=False` silent drop** (2026-07-20 qk-norm probe 사고): eval 로더가 ckpt-arch 추론(`load_state_dict(strict=False)`)을 쓰면 **새 opt-in 아키 요소(qk-norm 등)의 가중치가 unexpected로 조용히 드랍**되어 아키 불일치 inference. 사고: probe parvo 로더 2곳이 `QK_NORM_DEFAULT` 미설정 → qk ckpt q_norm/k_norm 124개 드랍, 07-18 S-full(qk) 후속 15잡(36854931~945) 전부 이 상태로 측정. 수정 = sd 키 자동 감지 + **드랍 방지 assert**(`unexpected`에 대상 키 잔존 시 즉사). **가드: 새 아키 flag를 pretrain에 추가하면 같은 turn에 eval 로더(자동 감지 + assert)도 배선**. `strict=False` 로더는 missing만이 아니라 unexpected도 검사.
