"""
Two-stream preprocessing: motion (M) and appearance (P) channels.

M channel (temporal change / motion pathway):
    - Input: image_previous, image_current
    - use_sobel=True:  [ΔL, Sobel_x(ΔL), Sobel_y(ΔL)] (3 channels)
    - use_sobel=False: [ΔL] (1 channel)   <- CoMP-MAE setting
    - ΔL: raw luminance difference in [-1, 1] (no normalization; the absolute
      magnitude of change is itself informative).

P channel (spatial structure / appearance pathway):
    - Input: image_current
    - use_sobel=True:  [∂x, ∂y, R, G, B] (5 channels)
    - use_sobel=False: [R, G, B] (3 channels, raw RGB)   <- CoMP-MAE setting
"""
...
class TwoStreamPreprocessing(nn.Module):
    ...
    def __init__(self, use_sobel: bool = True):
        ...
        # use_sobel=False (CoMP-MAE): no Sobel edge prior -> P=RGB(3ch), M=ΔL(1ch).
        #   This isolates the M/P two-stream effect on raw RGB input. use_sobel=True
        #   keeps the 5ch/3ch edge-prior variant.
    ...
    def compute_m_channel(
        ...
        ΔL keeps its absolute scale (no normalization) -- the magnitude of change
        is itself informative. Sobel(ΔL) captures the edges of the change map.
        ...
        # 2. Sobel on ΔL -- edges of the change map
        grad_x = F.conv2d(
        ...
        if not self.use_sobel:
            return delta_l  # CoMP-MAE: luminance-difference motion only (1ch)
    ...
    def compute_p_channel(self, image: torch.Tensor) -> torch.Tensor:
        ...
        """
        if not self.use_sobel:
            return image  # CoMP-MAE: raw RGB appearance only (3ch)
