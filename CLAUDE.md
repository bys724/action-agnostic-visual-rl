...
### 🔴 1차 BC-T 학습 cancel + 재제출 결정 (2026-04-30)

VideoMAE-ours BC-T (33615386) 학습 종료 후 로컬 H100에서 LIBERO rollout sanity 평가 → **0/50 = 0% SR** (10 task × 5 trial).

진단:
- ckpt 로드·inference 코드 모두 정상 (LIBERO 공식 evaluate.py/metric.py 분석 결과 일치)
- 비디오: 정책이 의도적으로 움직이지만 task 영역에서 점진 이탈
- 본질 원인: **`scripts/eval/finetune_libero_bct.py` 학습 cfg가 LIBERO 공식 default와 불일치**
  - LIBERO 공식 (`/opt/libero/libero/configs/data/default.yaml`): `use_joint=True`, `low_dim=["gripper_states", "joint_states"]`
  - 우리 학습 cfg: `use_joint=False`, `low_dim=["gripper_states"]`만
  - → `joint_states` (robot0_joint_pos, 7-d) 누락 = robot kinematics state 부재
  - 정책이 robot의 spatial configuration을 모름 → spatial control fail (비디오 패턴과 일치)
...
3. `obs_key_mapping`에 `joint_states: robot0_joint_pos`
4. (선택) `affine_translate=4` augmentation — LIBERO 공식 default

**클러스터 액션**: 33615385~33615392 cancel + 5 encoder × spatial × seed=0 새 cfg로 재제출. v11-VfromM (33615394/5)은 BC-T와 무관, 그대로 진행.

자세한 진단·수정 사항: [`docs/RESEARCH_PLAN.md`](docs/RESEARCH_PLAN.md) Phase 3-1 §"1차 BC-T 학습 중단 결정"

### 다음 단계

1. **🔴 BC-T 학습 driver 수정 + 5 encoder × spatial 재제출** (cluster 작업)
2. **새 BC-T ckpt 종료 시 로컬 워크스테이션에서 LIBERO rollout 재시도** — 인프라 준비됨 (`src/eval_libero.py` BC-T 전용 + libero-eval 컨테이너 + Dockerfile 패키지). 명령은 `docs/setup/LIBERO_TEST_GUIDE.md` 참고
3. **v11-VfromM full (33615395) 진행 중 ep4/ep8 probing 가능 시점 확인** (~5-6시간 후 시작 후)
4. **V-JEPA 별도 처리 결정** — 옵션 A (pre-extraction) vs 옵션 C (skip from main table). 새 1차 결과 본 후
