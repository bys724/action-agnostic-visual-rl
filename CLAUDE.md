...
**Paper main 활성 모델**: **Two-Stream v15** (Layered specialization, predictor-only V-JEPA + V-JEPA-M Option B + L_compose + 3-frame triple), **VideoMAE-ours** (controlled baseline). v6/v10/v11은 paper §method history 인용용.
...
**Phase 4 진입 — paper §5.1 main ablation C1 (v15 V-from-M) 학습 중. v15 V3 BC-T 18잡 완료.**

NeurIPS 2026 5월 마감. **Paper 전략 (5/12 결정)**: v15 only main + 5 baseline (VideoMAE-ours, DINOv2, SigLIP, VC-1). v11은 paper §method history 1-2 문장만 (paper main 제외). V-JEPA 2.1은 16-frame × 384² 입력 비용으로 BC main table 제외, probing only.
...
| **🏆 v15** (Layered specialization: predictor-only V-JEPA + V-JEPA-M Option B + L_compose + 3-frame triple) | **paper main** | ep50 완주 (34288968, 2026-05-12). ep32 `patch_mean_concat_p_t_p_tk` champion ★ |

**v15 핵심 발견 (2026-05-12)**:
- **`patch_mean_concat_p_t_p_tk` mode** = **+0.390 EgoDex** / **+0.584 LIBERO spatial gap=20** (v11 champion +0.288, VideoMAE +0.326 모두 추월)
- v11 ep44 동일 mode = +0.010 (거의 0) → P_t+P_tk가 v15 specific
- **M stream catalyst 가설**: 학습 중 M encoder가 P encoder에 motion-friendly 학습 압력 transfer → 학습 후 M 자체 망가져도 P_t+P_tk 강력. 통제군 DINOv2 +0.006으로 catalyst 인과 확정
- v15 학습 종반 P/M encoder CLS는 collapse, **patch 단위에선 healthy** (75% MAE recon 작동)
- v15 학습 cost: AIP_long 2×4 H100 × 42.91h = **343 GPU·h** (50ep 완주)
